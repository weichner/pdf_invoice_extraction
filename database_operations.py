...
from helpers import extract_data_invoice_a, extract_data_invoice_b
from schemas import InvoiceTypes, Invoices
...
async def get_one_invoice(invoice_id):
    invoice = invoices_db.select().where(invoices_db.c.invoice_id == invoice_id)
    result = await invoices_db.fetch_one(invoice)
    return result


async def get_all_invoices(doc):
    invoice = invoices_db.select()
    result = await invoices_db.fetchall(invoice)
    return result
